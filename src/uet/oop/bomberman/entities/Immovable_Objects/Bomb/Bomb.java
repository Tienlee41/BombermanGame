...
import uet.oop.bomberman.entities.Immovable_Objects.*;
...
import static uet.oop.bomberman.GameControll.GameStatus.WIN_ONE;
        ...
        if (bombStatus == BombStatus.EXPLODED) {
            ...
            for (int i = map.getMovingEntitiesList().size() - 1; i >= 0; i--) {
                ...
                if (insideBombRange_Pixel(xBomberPos + Sprite.SCALED_SIZE / 2, yBomberPos + Sprite.SCALED_SIZE / 2)) {
                    ((Movable_Object) map.getMovingEntitiesList().get(i)).setObjectStatus(Movable_Object.MovingObjectStatus.MORIBUND);
                }
            }
        }
    }
