...
import static uet.oop.bomberman.GameControll.GameStatus.*;
...
public class PlayingController extends SceneController implements Initializable {
    ...
    public void setGameController(GameControll gameControll) {
        super.setGameController(gameControll);
        currentLevel = gameControll.getCurrentLevel();
    }
        ...
        if (location.equals(playingURL)) {
            ...
            pauseButton.setOnAction(new EventHandler<ActionEvent>() {
                ...
                public void handle(ActionEvent event) {
                    GameControll.gameStatus = GAME_PAUSE;
            ...
            continueButton.setOnAction(new EventHandler<ActionEvent>() {
                ...
                public void handle(ActionEvent event) {
                    ...
                    GameControll.gameStatus = GAME_UNPAUSE;
                }
            });
            ...
            quitButton.setOnAction(new EventHandler<ActionEvent>() {
                ...
                public void handle(ActionEvent event) {
                    ...
                    if (buttonType.get() == ButtonType.YES) {
                        GameControll.gameStatus = GAME_LOSE;
                    } else GameControll.gameStatus = GAME_UNPAUSE;
                }
            });
            ...
            nextLevelTimeline.getKeyFrames().addAll(new KeyFrame(Duration.seconds(0),
                    event -> {
                        ...
                        if (timerCounter.get() <= 0) {
                            ...
                            GameControll.gameStatus = GAME_PLAYING;
            ...
            victoryTimeline.getKeyFrames().addAll(new KeyFrame(Duration.seconds(0),
                    event -> {
                        timerCounter.decrementAndGet();
                        if (timerCounter.get() <= 0) {
                            if (currentLevel.getLevelCode().get() <= GameControll.MAX_LEVEL
                                    && GameControll.gameStatus == WIN_ONE) {
                                GameControll.gameStatus = GameControll.GameStatus.GAME_START;
                            } else {
                                if (GameControll.gameStatus != GameControll.GameStatus.WIN_ALL)
                                    GameControll.gameStatus = GameControll.GameStatus.WIN_ALL;
                                else {
    ...
    public void updateStatus() throws SQLException {
        switch (GameControll.gameStatus) {
            case GAME_START:
                ...
                break;
            case GAME_PLAYING:
                if (curGamePoint < gameControll.getGamePoint()) {
                    pointText.setText(Integer.toString(++curGamePoint));
                } else if (curGamePoint > gameControll.getGamePoint()) {
                    pointText.setText(Integer.toString(--curGamePoint));
                }
                if (timeline.getStatus() == Timeline.Status.STOPPED) {
                    ...
                    maxBombs.setText(Integer.toString(((Bomber) gameControll.getCurrentLevel().getGameMap().getMovingEntitiesList().get(0)).getMaxBomb()));
            ...
            case WIN_ONE:
                ...
                if (victoryTimeline.getStatus() != Animation.Status.RUNNING) {
                    ...
                    gameControll.plusPoint((int) ((1 - progressBar.getProgress()) * (currentLevel.bonusRewardPoint() + 1) * 100));
                    timeline.stop();
            ...
            case WIN_ALL:
                if (victoryTimeline.getStatus() != Animation.Status.RUNNING) {
                    gameControll.editRankingSet(gameControll.getUsername(), gameControll.getGamePoint());
                    timerCounter.set(7);
                    victoryTimeline.playFromStart();
