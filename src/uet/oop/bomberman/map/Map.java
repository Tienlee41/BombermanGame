...
import uet.oop.bomberman.entities.Immovable_Objects.Grass;
...
//            BufferedReader br = new BufferedReader(inputStreamReader);
        ...
        for (int i = 0; i < heightTile; i++) {
            ...
            for (int j = 0; j < widthTile; j++) {
                ...
                switch (rowString.charAt(j)) {
                    case 'p':
                        Entity bomberman = new Bomber(j, i, new Collision(this, Bomber.WIDTH, Bomber.HEIGHT));
                    ...
                    case '1':
                        ...
                        break;
                    case '2':
                        Enemy oneal = new Oneal(j, i, new Collision(this, Oneal.WIDTH, Oneal.HEIGHT));
                        movingEntitiesList.add(oneal);
                        break;
                    case '3':
                        Enemy doll = new Doll(j, i, new Collision(this, Doll.WIDTH, Doll.HEIGHT));
                        movingEntitiesList.add(doll);
                        break;
                    case '4':
                        Enemy kondoria = new Kondoria(j, i, new Collision(this, Kondoria.WIDTH, Kondoria.HEIGHT));
     ...
     */
    public Graph convertMapToGraph(Vertices onealVertice) {
        ...
        Vertices bomberVertice = new Vertices((movingEntitiesList.get(0).getX() + Bomber.WIDTH / 2) / Sprite.SCALED_SIZE,
    ...
    public void updateMovingEntitiesList() {
        for (int i = movingEntitiesList.size() - 1; i >= 0; i--) {
            ...
            if (movingEntitiesList.get(i) instanceof Enemy) {
                if (((Movable_Object) movingEntitiesList.get(i)).getObjectStatus() == Movable_Object.MovingObjectStatus.DEAD) {
